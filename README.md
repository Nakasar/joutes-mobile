...
- **Marges de sécurité** (`--safe-top`, `--safe-bottom` dans `styles.css`) : à utiliser plutôt que `env(safe-area-inset-*)` en direct. iOS renseigne ses marges (encoche, barre gestuelle) ; **Android dessine sous la barre d'état sans la déclarer** — `env()` y vaut zéro. Un plancher de 24 px est donc posé pour Android seulement, la racine étant marquée `is-android` au démarrage (`src/lib/platform.ts`).
