...
- **Accueil** : ce qui bouge sur les jeux qu'on suit, en une requête (`GET /feed`, composée par le serveur avec les règles de l'accueil web) — puces des jeux suivis, directs en cours, sept prochains jours, fil à quatre sources (actualités, contenus de membres, decks, publications des réseaux d'un éditeur) plafonné pour qu'un éditeur bavard ne noie pas le reste, lieux suivis ou proches de la ville qu'on a dite, decks. Un visiteur lit tout, et on lui propose de dire où il joue
- **Actualités** : fil des news (bannières, jeux liés, likes), filtré sur les jeux suivis (`GET /news` accepte plusieurs `gameId`)
- **Recherche globale** : la loupe de l'accueil ouvre `/search` — jeux, cartes, lieux, événements, règles et politiques, cinq par groupe (`GET /search`), avec les dernières questions et les jeux en accès rapide
...
- **Valeur d'une collection** : l'écran « Collection » porte la valeur estimée de l'ensemble et celle de chaque jeu, la page d'un jeu la sienne, et la collection d'un groupe de jeu la sienne aussi — chacune avec son bouton « Recalculer ». C'est un relevé daté, calculé et enregistré par le serveur au moment où on le demande, et non un cours qui bouge tout seul : l'écran dit donc de quand il date, sur combien d'exemplaires cotés il repose, et signale que la collection a changé depuis
...
- **Événements** : les prochains événements en deux lectures — « Mes lieux » (ce que l'API croise pour un compte : lieux et jeux suivis, inscriptions, favoris) et « Autour de moi », pour qui a dit sa ville (`GET /geo/places`, rayon 1 à 150 km, mémorisée sur le téléphone : l'application ne lit pas le GPS). Un visiteur sans ville est invité à la dire plutôt que de lire une liste vide
- **Liste d'attente** : sur un évènement complet, la fiche propose de rejoindre la liste d'attente (`POST /events/{id}/waitlist`) et montre ensuite sa position. Quand une place se libère, la notification « Une place s'est libérée » (push et écran Notifications) ouvre la fiche, où l'on accepte la place ou la décline avant l'échéance (48 h par défaut, réglée par l'organisation). Quitter la file et décliner se confirment d'un second toucher : les deux font perdre sa place
- **Lieux** : l'annuaire, et la vitrine d'un lieu sur le modèle du site — état d'ouverture dans le hero, direct, annonces, événement à la une, prochains événements filtrés sur les jeux qu'on suit, informations pratiques (itinéraire par les coordonnées du lieu, distance depuis la ville dite, appel, courriel), agenda par mois ou en programme jour par jour avec ses inscriptions en tête, jeux avec le nombre d'événements à venir, à-propos avec photos, suivi
- **Jeux — fiche** : les réseaux de l'éditeur, son direct YouTube quand il diffuse (`GET /games/{id}/live`), ses publications rapatriées (`GET /games/{id}/social-posts`, derrière le fanion `socialFeed`), et le bouton Suivre (`PUT`/`DELETE /users/me/games/{id}`), repris sur les tuiles du catalogue
- **Couvertures** : les decks et les quizz montrent leur couverture (listes, cartes, fiche), et l'éditeur de deck la choisit parmi les cartes du deck
...
- **Tournois — puzzles** : sur une phase de type puzzle, l'en-tête montre un chronomètre parti de 0 au lieu du minuteur, l'onglet du match cède la place au puzzle à résoudre, à la table attribuée par l'organisation (elle arrive aussi en notification) et au bouton « j'ai terminé » (quand le self-report est activé), et le classement se lit au temps, le plus rapide en tête
...
- **Connexion** : par code OTP e-mail (Better Auth), session persistante ; les Réglages montrent l'avatar, le palier et les statuts que la session transporte
- **Navigation** : l'onglet ou la pastille choisis vivent dans l'URL (`useSearchParamState`) — revenir en arrière retrouve l'écran tel qu'on l'a quitté
...
- **Marges de sécurité** (`--safe-top`, `--safe-bottom` dans `styles.css`) : à utiliser plutôt que `env(safe-area-inset-*)` en direct. iOS renseigne ses marges (encoche, barre gestuelle) ; **Android dessine sous la barre d'état — et, depuis Android 15, sous la barre de navigation — sans les déclarer** : `env()` y vaut zéro. La racine est marquée `is-android` par `markPlatform()` (`src/lib/platform.ts`), appelé au démarrage depuis `src/main.tsx` avant le premier rendu, et `watchSystemInsets()` demande ensuite au natif les vraies marges (commande Tauri `android_system_insets`, `src-tauri/src/lib.rs`, qui lit `View.getRootWindowInsets()` par JNI et les convertit en pixels CSS) pour les poser en `--android-inset-top` / `--android-inset-bottom`. Le plancher de 24 px sur la barre d'état reste en repli tant qu'elles ne sont pas arrivées ; la barre de navigation (48 px à trois boutons, une vingtaine en gestuel) n'a pas de plancher — il serait faux dans l'un des deux modes.
